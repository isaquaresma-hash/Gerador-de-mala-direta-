...
# Função para aplicar a imagem de fundo e estilizar os blocos em padrão de verde
def carregar_configuracao_estilo(caminho_imagem):
    try:
        ...
        /* Ajuste do topo do contêiner principal */
        .block-container {{
...
try:
    ...
    # --- BARRA DE FILTROS LIMPA E INTUITIVA (3 COLUNAS) ---
    st.markdown('<div class="filter-header-badge">🔍 Consulta e Filtros</div>', unsafe_allow_html=True)

    c1, c2, c3 = st.columns([1, 1, 2]) # A coluna de município é 2x maior para melhor visualização

    # Identificação de colunas na planilha
    coluna_filiacao = next((col for col in df_original.columns if "filiad" in col.lower() or "situa" in col.lower() or "tipo" in col.lower()), None)
    ...
    with c3:
        ...
        if coluna_municipio:
            ...
            sel_mun = st.multiselect(
                ...
                options=opcoes_mun,
                placeholder="Todos os municípios selecionados (ou clique para filtrar)",
                key="sb_mun"
            )
            if sel_mun:
    ...
    # --- SELEÇÃO DE COLUNAS PARA A MALA DIRETA ---
    ...
    # Botões para facilitar a seleção de todas/nenhuma coluna
